...
export default function Budget({ spendings, budget, isOpen }) {
  return (
    <div
      className={`${!isOpen && "flex"} budget p-4 mb-8 rounded-2xl ${
        spendings < budget / 2
      ...
      {isOpen ? (
        <div >
          ...
          <div className="spendings">
            <span className="text-2xl font-semibold">{spendings}</span>
            <span className="text-md font-medium"> / {budget} DH</span>
          </div>
        </div>
      ) : (
